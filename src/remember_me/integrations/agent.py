...
class SovereignAgent:
    ...
    def run(self, user_input: str, context_str: str) -> Dict[str, Any]:
        ...
        # We assume the user wants an answer based on the tools.
        # But if "IMAGE" is present, we handle that separately or in parallel.

        final_response = self.engine.generate_response(user_input, augmented_context)

        # 4. IMAGE PHASE (Visualization)
        # We do this last so we can optionally use the *synthesized* description?
        # Or just the user prompt. User prompt is safer for now.
        if "IMAGE" in detected_intents:
            print(f"🎨 Orchestrator: Visualizing...")
            # Ideally, we ask the LLM to refine the prompt for the image generator.
            # "Describe the image to generate based on: {user_input}"
            img_prompt = user_input
            img_path = "dream_output.png"
            res = self.tools.generate_image(img_prompt, img_path)
